...
bool CmdRemoveSelectedSchematicItems::performExecute() {
  ...
  auto netSegmentItemsIterator = netSegmentItems.constBegin();
  while (netSegmentItemsIterator != netSegmentItems.constEnd()) {
    removeNetSegmentItems(*netSegmentItemsIterator.key(),
                          netSegmentItemsIterator.value().netpoints,
                          netSegmentItemsIterator.value().netlines,
                          netSegmentItemsIterator.value().netlabels);
    ++netSegmentItemsIterator;
  }
...
void CmdRemoveSelectedSchematicItems::removeNetSegmentItems(
  ...
  // Remove whole net segment
  removeNetSegment(netsegment);
  ...
  //                                CircuitIdentifier(forcedName));  // can
  //                                throw
  //        execNewChildCmd(cmdAddNetSignal);                        // can
  //        throw newNetSignal = cmdAddNetSignal->getNetSignal();
